...
export function responseGroups(rows: Row[], labels: string[] = []) {
  ...
  for (let row of rows.slice().sort((a,b)=>new Date(b.sentAt || 0).getTime()-new Date(a.sentAt || 0).getTime())) {
    if(['Joining Fight','JoiningFight','Defending','On The Way','OnTheWay'].includes(row.status)) row={...row,status:'Fighting'};
  ...
  for(const row of latest.values()) if(!['Not Sure','NotSure','Unsure'].includes(row.status)) (groups[row.status || 'Recorded'] ||= []).push({name:row.actorName || row.actorDiscordId || 'Unknown player',at:row.sentAt});
...
export function attackReports(alerts:Row[],responses:Row[]) {
  const reports=alerts.map(alert=>({id:String(alert._id),kind:'war',title:alert.payload?.message || 'Attack alert',at:alert.sentAt,
    groups:responseGroups(responses.filter(r=>r.reportId===String(alert._id) || (alert.payload?.messageId && (r.reportId===alert.payload.messageId || r.payload?.messageId===alert.payload.messageId))),['Fighting','Unavailable'])}));
