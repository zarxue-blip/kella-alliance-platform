...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="/assets/fantasy-portal.css?v=base-preview-1" />
  </head>
  <body>
    ...
    <a class="creator-companion" href="https://paypal.me/exuzz" target="_blank" rel="noopener noreferrer" aria-label="Support Creator on PayPal"><span class="creator-bubble">Support Creator</span><picture><source media="(prefers-reduced-motion: reduce)" srcset="/assets/support-creator-still.png"/><img src="/assets/support-creator.webp" alt="" width="144" height="256" decoding="async"/></picture></a>
    <div class="shell">
    ...
    <script>
      ...
      function pathRequiresAdmin(path) {
        return ["/migration/admin", "/officer", "/tools", "/events", "/alerts", "/shield-alerts", "/embed-sender", "/complaints", "/settings"].some(function(prefix) {
      ...
      function renderCalendarCell(date, type, items) {
        ...
        const entries = visible.map(function(item) {
          return '<span class="calendar-entry"><span>' + escapeHtml(item.title || "Event") + '</span><small>' + escapeHtml(item.meta || "") + '</small></span>';
        }).join("");
      ...
      function boardCurrentEvent(events, now = new Date()) {
        const ordered = sortedEvents(events).filter(function(event) { return Number.isFinite(new Date(event.startsAt).getTime()); });
        const today = ordered.filter(function(event) { return dayKey(event.startsAt) === dayKey(now); });
...
${portalHomeClient}
      ...
      async function loadHomeShowcase() {
        ...
        try {
          const data = await fetchJson('/api/dashboard/champions?metric='+metric);
      ...
      async function renderAttendance() {
        ...
        try {
          const results = await Promise.all([loadDashboardEvents(), loadPolls(true), loadPersonalAttendance()]);
      ...
      async function embedToolContent() {
        ...
        return (channelsError || templatesError ? '<div class="error" style="margin-top:18px">' + escapeHtml(channelsError || templatesError) + '. Add your Admin Key in Settings if needed.</div>' : '') +
          '<section class="two" style="margin-top:18px"><div class="card stack"><div class="card-header"><h3>Embed Sender</h3><div class="toolbar"><button class="secondary" data-action="preview-embed">Preview</button><button class="secondary" data-action="save-template">Save Template</button><button class="danger" data-action="delete-template">Delete Template</button><button class="primary" data-action="send-embed">Send Embed</button></div></div><div class="form-grid">' +
            ...
            '<label class="wide">Embed Description<textarea data-embed="description">Our next alliance event is coming up. Check the calendar and share your availability.</textarea></label>' +
      ...
      function renderAdminAccessRequired() {
        app.innerHTML =
          ...
          '<section class="card">' + empty("Admin tools require an authorized KOG officers/admin Discord account.") + '<a href="/" data-link>Return Home</a>'  + '</section>';
      }
      ...
      async function route() {
        ...
        if (path !== "/" && path !== "/migration" && !state.auth?.authenticated) {
          app.innerHTML = pageHeader("Log in to continue", "Sign in with Discord to open alliance features.", '<button class="primary" data-action="discord-login">Login with Discord</button><a class="secondary" href="/" data-link>Return Home</a>');
          return;
        }
