...
export const portalHomeClient = String.raw`
      function renderAllianceBoard(events = []) {
        ...
        const coins = Array.from({length:14}, function(_,i) { return '<i class="portal-coin" style="--x:'+([6,19,39,58,72,88,94,52,83,31,65,9,91,48][i])+'%;--y:'+([19,66,12,31,9,45,76,81,65,88,55,85,17,48][i])+'%;--size:'+(18+(i*11)%38)+'px;--duration:'+(7+i%6)+'s;--delay:-'+i+'s"><span>✦</span></i>'; }).join('');
        return '<div class="portal-home"><section class="portal-hero" aria-labelledby="portal-title"><div class="portal-scenery" aria-hidden="true"></div><div class="portal-coins" aria-hidden="true">'+coins+'</div><div class="portal-hero-copy"><p class="portal-eyebrow"><span></span> CALL OF DRAGONS · REALM 881</p><h1 id="portal-title"><em>KELLA</em></h1><p class="portal-subtitle">Alliance Platform</p><p class="portal-intro">One banner. Countless adventures.<br>Your alliance, always within reach.</p><div class="portal-actions"><a class="primary" href="/calendar" data-link>Alliance Calendar <span>↗</span></a><a class="secondary" href="/members" data-link>Meet the alliance</a></div><a class="portal-archive-link" href="/wiki" data-link>Explore the archives <span>→</span></a></div><div class="portal-character"><img data-portal-kella src="/assets/kella-character.png" alt="Kella, our halfling caravan keeper" width="340" height="390" fetchpriority="high"/><div class="portal-character-label"><span>YOUR CARAVAN KEEPER</span><strong>Kella</strong><small>Good company. Better coin.</small></div></div><div class="portal-hero-foot"><span>KELLA <b>◆</b> REALM 881</span><a href="#portal-explore">Discover the realm ↓</a></div></section><div class="portal-body"><section class="portal-current"><span class="portal-eyebrow">'+(current?'ON THE HORIZON':'ALLIANCE CALENDAR')+'</span><h2>'+escapeHtml(current?.title || 'The next adventure awaits')+'</h2><p>'+(current?formatUtcDateTime(current.startsAt):'Visit the calendar for the latest alliance plans.')+'</p><a href="'+(current?'/attendance/'+encodeURIComponent(current.id):'/calendar')+'" data-link>'+(current?'View event':'Open calendar')+' →</a></section><section class="portal-section" id="portal-explore"><div class="portal-section-heading"><div><p class="portal-eyebrow">EVERYTHING FOR THE JOURNEY</p><h2>Explore the realm</h2></div><span>Plan. Prepare. Prevail.</span></div><div class="portal-features">'+destinations.map(function(item,index){return '<a class="portal-feature" href="'+item[0]+'" data-link><span class="portal-feature-number">0'+(index+1)+'</span><img src="/assets/'+item[1]+'" alt="" width="62" height="62" loading="lazy"/><strong>'+item[2]+'</strong><p>'+item[3]+'</p><span class="portal-feature-arrow">↗</span></a>';}).join('')+'</div></section><div class="portal-live" data-portal-live></div><section class="portal-section"><div class="portal-section-heading"><div><p class="portal-eyebrow">WRITTEN UNDER ONE BANNER</p><h2>Alliance Chronicle</h2></div><a href="/calendar" data-link>All events →</a></div><div class="portal-chronicle">'+events.filter(function(e){return Number.isFinite(Date.parse(e.startsAt));}).slice().sort(function(a,b){return Date.parse(b.startsAt)-Date.parse(a.startsAt);}).slice(0,3).map(function(e){return '<a class="portal-story" href="/attendance/'+encodeURIComponent(e.id)+'" data-link><time>'+formatUtcDateTime(e.startsAt)+'</time><h3>'+escapeHtml(e.title)+'</h3><p>'+escapeHtml(e.description || 'View the event and alliance attendance.')+'</p><span>Read event →</span></a>';}).join('')+(events.length?'':'<div class="portal-empty">Our next chapter is still being written. Scheduled events will appear here.</div>')+'</div></section><section class="portal-section portal-community"><div class="portal-champions"></div><div class="portal-participation" data-portal-participation><p class="portal-eyebrow">STAND WITH YOUR ALLIANCE</p><h2>Every voice counts.</h2><p>Check attendance, take part in polls, and help shape our next move.</p><a class="secondary" href="/attendance" data-link>Attendance & polls →</a></div></section><footer class="portal-footer"><img src="/assets/kella-logo.png" width="40" height="40" alt=""/><div><strong>KELLA</strong><p>Realm 881 · Built for our alliance.</p></div><a href="/profile" data-link>Your profile ↗</a></footer></div></div>';
      }
      ...
      function renderPortalUpcoming(events) {
        const upcoming = events.filter(function(e){return Date.parse(e.startsAt) >= Date.now();}).sort(function(a,b){return Date.parse(a.startsAt)-Date.parse(b.startsAt);}).slice(0,3);
        if (!upcoming.length) return '';
        return '<section class="portal-section"><div class="portal-section-heading"><div><p class="portal-eyebrow">RALLY UNDER OUR BANNER</p><h2>Upcoming events</h2></div><a href="/calendar" data-link>Full calendar →</a></div><div class="portal-chronicle">'+upcoming.map(function(e){return '<a class="portal-story" href="/attendance/'+encodeURIComponent(e.id)+'" data-link><time>'+formatUtcDateTime(e.startsAt)+'</time><h3>'+escapeHtml(e.title)+'</h3><span>View event & attendance →</span></a>';}).join('')+'</div></section>';
      }
      ...
      (function initializeHeaderCoins() {
        ...
        function resize() {
          ...
          const count=width<700?420:2000;
